...
results_dir = "./../results/baseline/"+str(prob_threshold)+"/"+folder
...
for i in range(1, len(res_list)+1):
    ...
    for row in orf_test.iterrows():
        ...
        if pred_proba>prob_threshold:          
            ...
            if cate>0: # positive uplift
                df_gains_without_res_2_1 = df_gains_without_res_2_1.append(list(row)[1],)
                list_pred_proba.append([pred_proba])
                sorted_pred_proba_list = sorted(list_pred_proba, key=lambda x: x[0],reverse=True)
                if sorted_pred_proba_list:                
                    max_pred_proba = sorted_pred_proba_list[0][0]
                    if max_pred_proba > 0 and nr_res:
